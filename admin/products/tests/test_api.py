...
def test_retrieve_product(client, product):
  response = client.get(f"/admin/api/products/{product.id}/")

  assert response.status_code == 200
  assert response.data['title'] == "Laptop"

def test_update_product(client, product):
  response = client.put(f"/admin/api/products/{product.id}/",
                        {
                          "title": "Gaming Laptop",
...
def test_delete_product(client, product):
  response = client.delete(f"/admin/api/products/{product.id}/")
...
def test_retrieve_invalid_product(client):
    response = client.get("/admin/api/products/9999/")
...
def test_delete_invalid_product(client):
    response = client.delete("/admin/api/products/9999/")
